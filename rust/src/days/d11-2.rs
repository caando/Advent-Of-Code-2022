    ...
    fn throw_items(&mut self) {
        ...
        for i in 0..self.len() {
            while !self[i].items.is_empty() {
                ...
                let catcher = if worry % self[i].divisor == 0 {self[i].first_monkey} else {self[i].second_monkey};
                self[catcher].catch_item(worry);
            }
        }
    }
}
...
fn main() -> io::Result<()> {
    ...
    println!("{}", count_inspects_of_highest_2(parse_monkeys(lines), 10000));
    Ok(())
}
